...
function UserSubscriptionCreatePage() {
  ...
  const handleSubmit = async (values: UserSubscriptionInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await roqClient.user_subscription.create({ data: values as RoqTypes.user_subscription });
      resetForm();
  ...
  const formik = useFormik<UserSubscriptionInterface>({
    initialValues: {
      ...
      subscription_renewal_date: new Date(new Date().toDateString()),
      subscription_tier_id: 0,
      subscription_payment_status: false,
      user_subscription_type: '',
      user_subscription_end_reason: '',
      user_subscription_renewal_reminder: false,
      user_subscription_last_updated: new Date(new Date().toDateString()),
      user_subscription_activation_date: new Date(new Date().toDateString()),
      user_subscription_cancellation_date: new Date(new Date().toDateString()),
      user_subscription_pause_date: new Date(new Date().toDateString()),
      user_subscription_pause_reason: '',
      user_subscription_pause_duration: 0,
      user_subscription_pause_end_date: new Date(new Date().toDateString()),
      user_subscription_pause_count: 0,
      user_subscription_pause_extension: 0,
      user_subscription_feedback: '',
    },

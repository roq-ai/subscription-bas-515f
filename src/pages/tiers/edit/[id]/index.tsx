...
function TierEditPage() {
  ...
  const { data, error, isLoading, mutate } = useTierFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: TierInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.tier.update({
        data: values as RoqTypes.tier,
        where: {
          id,
        },
      });

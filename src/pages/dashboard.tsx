...
import { useRoqPlatformUserProfiles, useMessageCount, useSubscriptionCount, useTierCount } from 'lib/roq';

import { UserSubscriptionListPage as ShowCaseList } from 'pages/user-subscriptions';

function DashboardPage() {
  const { session } = useSession();

  const messageStat = useMessageCount();

  const subscriptionStat = useSubscriptionCount();

  const tierStat = useTierCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Messages',
      result: messageStat?.data,
      isLoading: messageStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Subscriptions',
      result: subscriptionStat?.data,
      isLoading: subscriptionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Tiers',
      result: tierStat?.data,
      isLoading: tierStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest User Subscriptions
                </Text>
                <NextLink href={`user-subscriptions`} passHref legacyBehavior>
                  <Button
